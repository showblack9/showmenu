...
public static class PlatformSpecificData_Serialize
{
    ...
    public static void Prefix(PlatformSpecificData __instance)
    {
        ...
        ShowSpoof.spoofPlatform(__instance);
...
public static class SystemInfo_deviceUniqueIdentifier_Getter
{
    ...
    {
        if (ShowMenu.spoofDeviceId.Value){
...
{
    // Postfix patch of VersionShower.Start to show ShowMenu version
    public static void Postfix(VersionShower __instance)
    {
        if (ShowMenu.supportedAU.Contains(Application.version)){ // Checks if Among Us version is supported

            __instance.text.text =  $"ShowMenu v{ShowMenu.showVersion} (v{Application.version})"; // Supported
        
        }else{

            __instance.text.text =  $"ShowMenu v{ShowMenu.showVersion} (<color=red>v{Application.version}</color>)"; //Unsupported
...
public static class PingTracker_Update
{
    ...
    public static void Postfix(PingTracker __instance)
    {
        ...
        if (AmongUsClient.Instance.IsGameStarted){
            ...
            __instance.text.text = $"ShowMenu by showblack ~ {Utils.getColoredPingText(AmongUsClient.Instance.Ping)}";
            
            return;
        }

        __instance.text.text = $"ShowMenu by showblack\n{Utils.getColoredPingText(AmongUsClient.Instance.Ping)}";
...
public static class Mushroom_FixedUpdate
{
    ...
    {
        ShowESP.sporeCloudVision(__instance);
    }
}
